...
func (w *wizard) deployNode(boot bool) {
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
		...
		} else if w.conf.Genesis.Config.Clique != nil || w.conf.Genesis.Config.WPoa != nil {
			...
			// Clique or WPoa based signers need a keyfile and unlock password, ask if unavailable
			if infos.keyJSON == "" {
				fmt.Println()
