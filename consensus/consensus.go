...
type Handler interface {
	...
	HandleMsg(address common.Address, msg p2p.Msg) (bool, error)

	SubscribeNewMsgEvent(ch chan<- PbftMsg) event.Subscription
}

type IstHandler interface {
...
// dbft is a dpos consensus engine to avoid byzantine failure (dpos-bft)
type Dbft interface {
	Engine
