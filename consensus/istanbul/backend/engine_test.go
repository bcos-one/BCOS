...
func makeBlock(chain *core.BlockChain, engine *backend, parent *types.Block) *types.Block {
	...
	var results = make(chan *types.Block);
	_ = engine.Seal(chain, block, results, nil)
	findBlock := <-results
	return findBlock
}
...
func TestSealCommitted(t *testing.T) {
	...
	select {
	case <-time.After(delay):
	case resultBolck := <-results:
		if resultBolck.Hash() != expectedBlock.Hash() {
			t.Errorf("hash mismatch: have %v, want %v", resultBolck.Hash(), expectedBlock.Hash())
		}
	}
