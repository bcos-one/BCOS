...
func NewEVMContext(msg Message, header *types.Header, chain ChainContext, author *common.Address) vm.Context {
	...
	if author == nil {
		beneficiary, _ = chain.Engine().Author(header) // Ignore error, we're past header va lidation
	} else {
		beneficiary = *author
	}
	return vm.Context{
...
// CanTransfer checks whether there are enough funds in the address' account to make a transfer.
// This does not take the necessary gas in to account to make the transfer valid.
func CanTransferToken(db vm.StateDB, addr common.Address, token common.Address, amount *big.Int) bool {
	return db.GetTokenBalance(addr, token).Cmp(amount) >= 0
}

// Transfer subtracts amount from sender and adds amount to recipient using the given Db
func TransferToken(db vm.StateDB, sender, recipient common.Address, token common.Address, amount *big.Int) {
