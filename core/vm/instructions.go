...
func opBalance(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	slot := stack.peek()
	if contract.Token != nil {
		slot.Set(interpreter.evm.StateDB.GetTokenBalance(common.BigToAddress(slot), *contract.Token))
	} else {
...
func opCreate(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	contract.UseGas(gas)
	res, addr, returnGas, suberr := interpreter.evm.Create(contract, input, gas, contract.Token, value)
...
func opCreate2(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	gas -= gas / 64
	contract.UseGas(gas)
	res, addr, returnGas, suberr := interpreter.evm.Create2(contract, input, gas, endowment, salt, contract.Token)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnGas, err := interpreter.evm.Call(contract, toAddr, args, gas, contract.Token, value)
	if err != nil {
		stack.push(interpreter.intPool.getZero())
	} else {
...
func opCallCode(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnGas, err := interpreter.evm.CallCode(contract, toAddr, args, gas, contract.Token, value)
	if err != nil {
		stack.push(interpreter.intPool.getZero())
	} else {
		stack.push(interpreter.intPool.get().SetUint64(1))
...
func opDelegateCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnGas, err := interpreter.evm.DelegateCall(contract, toAddr, args, gas, contract.Token)
	if err != nil {
		stack.push(interpreter.intPool.getZero())
	} else {
		stack.push(interpreter.intPool.get().SetUint64(1))
...
func opStaticCall(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnGas, err := interpreter.evm.StaticCall(contract, toAddr, args, gas, contract.Token)
	if err != nil {
		stack.push(interpreter.intPool.getZero())
	} else {
		stack.push(interpreter.intPool.get().SetUint64(1))
...
func opSuicide(pc *uint64, interpreter *EVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	if contract.Token != nil {
		balance := interpreter.evm.StateDB.GetTokenBalance(contract.Address(), *contract.Token)
		interpreter.evm.StateDB.AddTokenBalance(common.BigToAddress(stack.pop()), *contract.Token, balance)
	} else {
