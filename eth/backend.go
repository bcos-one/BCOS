...
import (
	"errors"
	"fmt"
	"github.com/bcos-one/BCOS/consensus/istanbul"
	"github.com/bcos-one/BCOS/consensus/wpoa"
	"github.com/bcos-one/BCOS/crypto"
	"github.com/bcos-one/BCOS/params/global"
	"math/big"
	"runtime"
	"sync"
	"sync/atomic"
	...
	"github.com/bcos-one/BCOS/consensus/clique"
	"github.com/bcos-one/BCOS/consensus/ethash"
	"github.com/bcos-one/BCOS/core"
	"github.com/bcos-one/BCOS/core/bloombits"
	...
	"github.com/bcos-one/BCOS/eth/filters"
	"github.com/bcos-one/BCOS/eth/gasprice"
	"github.com/bcos-one/BCOS/ethdb"
	"github.com/bcos-one/BCOS/event"
	"github.com/bcos-one/BCOS/internal/ethapi"
	"github.com/bcos-one/BCOS/log"
	"github.com/bcos-one/BCOS/miner"
	"github.com/bcos-one/BCOS/node"
	"github.com/bcos-one/BCOS/p2p"
	"github.com/bcos-one/BCOS/params"
	"github.com/bcos-one/BCOS/rlp"
	"github.com/bcos-one/BCOS/rpc"
	dbftBackend "github.com/bcos-one/BCOS/consensus/dbft/backend"
	istanbulBackend "github.com/bcos-one/BCOS/consensus/istanbul/backend"
)
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	eth.miner.SetExtra(makeExtraData(config.MinerExtraData, eth.chainConfig.IsBcos))
...
func makeExtraData(extra []byte, isBcos bool) []byte {
	if len(extra) == 0 {
	...
	if uint64(len(extra)) > params.GetMaximumExtraDataSize(isBcos) {
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Dbft != nil {
		return dbftBackend.New(chainConfig.Dbft, ctx.NodeKey(), db)
	}
...
func (s *Ethereum) APIs() []rpc.API {
	...
	if global.IsGeth() {
		...
		apis = ethapi.GetAPIs_BCOS(s.APIBackend)
	}
