...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress), acc1Addr, nil, big.NewInt(10000), params.TxGas, nil, nil), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		nonce++
		tx3, _ := types.SignTx(types.NewContractCreation(nonce, nil, big.NewInt(0), 1000000, big.NewInt(0), testContractCode), signer, acc1Key)
