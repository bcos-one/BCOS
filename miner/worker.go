...
func (w *worker) start() {
	...
	if dbft, ok := w.engine.(consensus.Dbft); ok {
		dbft.Start(w.chain, w.chain.CurrentBlock, w.chain.HasBadBlock)
	}
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case req := <-w.newWorkCh:
			if h, ok := w.engine.(consensus.IstHandler); ok {
				h.NewChainHead()
			}
			w.commitNewWork(req.interrupt, req.noempty, req.timestamp)
			if w.config.Dbft != nil {
				timer.Reset(time.Duration(w.config.Dbft.BlockPeriod) * time.Second)
			}
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				if w.config.WPoa != nil && w.config.WPoa.Period == 0 {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if err := w.engine.Prepare(w.chain, header); err != nil {
		if _, ok := w.engine.(consensus.Dbft); ok {
